...
\justify
\section{Parties}
\subsection{$CLIENTCOMPANY${\getvalue{name}}}
\AUDITORCOMPANY{\getvalue{name}}
\AUDITORLOGO{\getvalue{name}}
\VERSIONDOCUMENT{\getvalue{name}}
\clientaddress{\getvalue{name}}
\clientcompany{\getvalue{name}}
\clientname{\getvalue{name}}
\clientproduit{\getvalue{name}}
\clientrepresentative{\getvalue{name}}
\clientsiren{\getvalue{name}}
\clienttitle{\getvalue{name}}
\mission{\getvalue{name}}
L'entreprise \textit{\CLIENTCOMPANY{\getvalue{name}}} (SIREN: \textit{$CLIENTSIREN$}), dont le siège social est situé à \textit{$CLIENTADDRESS$}, représentée par \textit{$CLIENTREPRESENTATIVE$}, en qualité de \textit{$CLIENTTITLE$}.
